...
public class Species {
    ...
    public Species(String s_name, ExpCurve s_curve, Type s_type1, Type s_type2,
            int s_baseHP, int s_baseAtk, int s_baseDef, int s_baseSpcAtk, int s_baseSpcDef,
    ...
    static {
        ...
        for(int i = 0; i < allSpecies.length; i++) {
            ...
            int s_baseSpcAtk = Constants.basestats[i][4];
            int s_baseSpcDef = Constants.basestats[i][5];
            int s_killExp = Constants.killexps[i];
            int s_pokedexNum = i;
            
            s = new Species(s_name, s_curve, s_type1, s_type2, s_baseHP, s_baseAtk, 
                    s_baseDef, s_baseSpcAtk, s_baseSpcDef, s_baseSpd, s_killExp, s_pokedexNum);
            allSpecies[i] = s;
        }
    }
    ...
    public String toString() {
        return String.format("%d %s %s %s%s Stats: %d %d %d %d %d %d Exp: %d", pokedexNum, name, curve, type1,
                ((type2 == Type.NONE) ? "" : " " + type2), baseHP, baseAtk, baseDef, baseSpd,
                baseSpcAtk, baseSpcDef, killExp);
